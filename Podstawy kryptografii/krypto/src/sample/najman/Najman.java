...
public class Najman extends Number implements Comparable<Najman> {
     ...
     */
    public Najman(int signum, byte[] magnitude) {
     ...
     */
    private Najman(int signum, int[] magnitude) {
     ...
     * @see    Character#digit
     */
    public Najman(String val, int radix) {
     ...
     */
    Najman(char[] val, int sign, int len) {
        int cursor = 0, numDigits;
     ...
     * @see #bitLength()
     */
    public Najman(int numBits, Random rnd) {
     ...
     * @see    #bitLength()
     */
    public Najman(int bitLength, int certainty, Random rnd) {
        Najman prime;
     ...
     * @since 1.4
     */
    public static Najman probablePrime(int bitLength, Random rnd) {
        if (bitLength < 2)
            throw new ArithmeticException("bitLength < 2");
     ...
     */
    private static Najman smallPrime(int bitLength, int certainty, Random rnd) {
    ...
    private static final Najman SMALL_PRIME_PRODUCT
     ...
     * test.
     */
    private static Najman largePrime(int bitLength, int certainty, Random rnd) {
        Najman p;
        p = new Najman(bitLength, rnd).setBit(bitLength-1);
        p.mag[p.mag.length-1] &= 0xfffffffe;

        // Use a sieve length likely to contain the next prime number
        int searchLen = getPrimeSearchLen(bitLength);
        NajmanoweSito searchSieve = new NajmanoweSito(p, searchLen);
        Najman candidate = searchSieve.retrieve(p, certainty, rnd);

        while ((candidate == null) || (candidate.bitLength() != bitLength)) {
            p = p.add(Najman.valueOf(2*searchLen));
            if (p.bitLength() != bitLength)
                p = new Najman(bitLength, rnd).setBit(bitLength-1);
            p.mag[p.mag.length-1] &= 0xfffffffe;
            searchSieve = new NajmanoweSito(p, searchLen);
     ...
     * @since 1.5
     */
    public Najman nextProbablePrime() {
        if (this.signum < 0)
        ...
        while (true) {
            NajmanoweSito searchSieve = new NajmanoweSito(result, searchLen);
            Najman candidate = searchSieve.retrieve(result,
                    DEFAULT_PRIME_CERTAINTY, null);
            if (candidate != null)
                return candidate;
            result = result.add(Najman.valueOf(2 * searchLen));
        }
    }
    ...
    private boolean passesLucasLehmer() {
        Najman thisPlusOne = this.add(ONE);
        ...
        // Step 2
        Najman u = lucasLehmerSequence(d, thisPlusOne, this);
     ...
     */
    private static int jacobiSymbol(int p, Najman n) {
        if (p == 0)
            return 0;
        ...
        if ((p & u & 2) != 0)   // p = u = 3 (mod 4)?
            j = -j;
        // And reduce u mod p
        u = n.mod(Najman.valueOf(p)).intValue();
    ...
    private static Najman lucasLehmerSequence(int z, Najman k, Najman n) {
        Najman d = Najman.valueOf(z);
        Najman u = ONE; Najman u2;
        Najman v = ONE; Najman v2;
    ...
    private boolean passesMillerRabin(int iterations, Random rnd) {
        ...
        Najman thisMinusOne = this.subtract(ONE);
        Najman m = thisMinusOne;
        ...
        for (int i=0; i < iterations; i++) {
            ...
            Najman b;
            do {
                b = new Najman(this.bitLength(), rnd);
            } while (b.compareTo(ONE) <= 0 || b.compareTo(this) >= 0);

            int j = 0;
            Najman z = b.modPow(m, this);
     ...
     */
    Najman(int[] magnitude, int signum) {
     ...
     */
    private Najman(byte[] magnitude, int signum) {
        this.signum = (magnitude.length == 0 ? 0 : signum);
        this.mag = stripLeadingZeroBytes(magnitude);
        if (mag.length >= MAX_MAG_LENGTH) {
            checkRange();
     ...
     */
    public static Najman valueOf(long val) {
     ...
     */
    private static Najman valueOf(int val[]) {
        return (val[0] > 0 ? new Najman(val, 1) : new Najman(val));
    }
    ...
    private static Najman posConst[] = new Najman[MAX_CONSTANT+1];
    private static Najman negConst[] = new Najman[MAX_CONSTANT+1];
     ...
     */
    private static volatile Najman[][] powerCache;
    ...
    static {
        for (int i = 1; i <= MAX_CONSTANT; i++) {
            ...
            magnitude[0] = i;
            posConst[i] = new Najman(magnitude,  1);
            negConst[i] = new Najman(magnitude, -1);
        }
         ...
         * on demand.
         */
        powerCache = new Najman[Character.MAX_RADIX+1][];
        logCache = new double[Character.MAX_RADIX+1];

        for (int i=Character.MIN_RADIX; i <= Character.MAX_RADIX; i++) {
            powerCache[i] = new Najman[] { Najman.valueOf(i) };
     ...
     * @since   1.2
     */
    public static final Najman ZERO = new Najman(new int[0], 0);
     ...
     * @since   1.2
     */
    public static final Najman ONE = valueOf(1);

    /**
     * The Matma constant two.  (Not exported.)
     */
    public static final Najman TWO = valueOf(2);

    /**
     * The Matma constant -1.  (Not exported.)
     */
    private static final Najman NEGATIVE_ONE = valueOf(-1);
     ...
     * @since   1.5
     */
    public static final Najman TEN = valueOf(10);
     ...
     */
    public Najman add(Najman val) {
        if (val.signum == 0)
            return this;
        if (signum == 0)
            return val;
        if (val.signum == signum)
            return new Najman(add(mag, val.mag), signum);
        ...
        return new Najman(resultMag, cmp == signum ? 1 : -1);
    }
    ...
    Najman add(long val) {
        ...
        if (Long.signum(val) == signum)
            return new Najman(add(mag, Math.abs(val)), signum);
        int cmp = compareMagnitude(val);
        if (cmp == 0)
            return ZERO;
     ...
     * @return {@code this - val}
     */
    public Najman subtract(Najman val) {
        if (val.signum == 0)
            return this;
        if (signum == 0)
            return val.negate();
     ...
     * @return {@code this * val}
     */
    public Najman multiply(Najman val) {
        ...
        if ((xlen < KARATSUBA_THRESHOLD) || (ylen < KARATSUBA_THRESHOLD)) {
            ...
            return new Najman(result, resultSign);
        } else {
    ...
    private static Najman multiplyByInt(int[] x, int y, int sign) {
        if (Integer.bitCount(y) == 1) {
            return new Najman(shiftLeft(x,Integer.numberOfTrailingZeros(y)), sign);
        }
        int xlen = x.length;
     ...
     */
    Najman multiply(long v) {
        if (v == 0 || signum == 0)
            return ZERO;
        if (v == DNajman.INFLATED)
            return multiply(Najman.valueOf(v));
     ...
     */
    private static Najman multiplyKaratsuba(Najman x, Najman y) {
        ...
        Najman xl = x.getLower(half);
        Najman xh = x.getUpper(half);
        Najman yl = y.getLower(half);
        Najman yh = y.getUpper(half);

        Najman p1 = xh.multiply(yh);  // p1 = xh*yh
        Najman p2 = xl.multiply(yl);  // p2 = xl*yl

        // p3=(xh+xl)*(yh+yl)
        Najman p3 = xh.add(xl).multiply(yh.add(yl));

        // result = p1 * 2^(32*2*half) + (p3 - p1 - p2) * 2^(32*half) + p2
        Najman result = p1.shiftLeft(32*half).add(p3.subtract(p1).subtract(p2)).shiftLeft(32*half).add(p2);
     ...
     *
     */
    private static Najman multiplyToomCook3(Najman a, Najman b) {
        ...
        Najman v0, v1, v2, vm1, vinf, t1, t2, tm1, da1, db1;
        ...
        Najman result = vinf.shiftLeft(ss).add(t2).shiftLeft(ss).add(t1).shiftLeft(ss).add(tm1).shiftLeft(ss).add(v0);
     ...
     * numbers.
     */
    private Najman getToomSlice(int lowerSize, int upperSize, int slice,
                                int fullsize) {
        ...
        return new Najman(trustedStripLeadingZeroInts(intSlice), 1);
    }
    ...
    private Najman exactDivideBy3() {
        ...
        result = trustedStripLeadingZeroInts(result);
        return new Najman(result, signum);
    }
    ...
    private Najman getLower(int n) {
        ...
        return new Najman(trustedStripLeadingZeroInts(lowerInts), 1);
    }
    ...
    private Najman getUpper(int n) {
        ...
        return new Najman(trustedStripLeadingZeroInts(upperInts), 1);
    }
    ...
    private Najman square() {
        ...
        if (len < KARATSUBA_SQUARE_THRESHOLD) {
            ...
            return new Najman(trustedStripLeadingZeroInts(z), 1);
        } else {
     ...
     * squareToLen.
     */
    private Najman squareKaratsuba() {
        int half = (mag.length+1) / 2;

        Najman xl = getLower(half);
        Najman xh = getUpper(half);

        Najman xhs = xh.square();  // xhs = xh^2
        Najman xls = xl.square();  // xls = xl^2
     ...
     */
    private Najman squareToomCook3() {
        int len = mag.length;
        ...
        Najman v0, v1, v2, vm1, vinf, t1, t2, tm1, da1;
     ...
     */
    public Najman divide(Najman val) {
     ...
     * @return {@code this / val}
     * @throws ArithmeticException if {@code val} is zero.
     * @see ZmiennyNajman#divideKnuth(ZmiennyNajman, ZmiennyNajman, boolean)
     */
    private Najman divideKnuth(Najman val) {
        ZmiennyNajman q = new ZmiennyNajman(),
                a = new ZmiennyNajman(this.mag),
                b = new ZmiennyNajman(val.mag);
     ...
     * @throws ArithmeticException if {@code val} is zero.
     */
    public Najman[] divideAndRemainder(Najman val) {
        if (val.mag.length < BURNIKEL_ZIEGLER_THRESHOLD ||
                mag.length - val.mag.length < BURNIKEL_ZIEGLER_OFFSET) {
    ...
    /** Long division */
    private Najman[] divideAndRemainderKnuth(Najman val) {
        Najman[] result = new Najman[2];
        ZmiennyNajman q = new ZmiennyNajman(),
                a = new ZmiennyNajman(this.mag),
                b = new ZmiennyNajman(val.mag);
        ZmiennyNajman r = a.divideKnuth(b, q);
     ...
     * @throws ArithmeticException if {@code val} is zero.
     */
    public Najman remainder(Najman val) {
        if (val.mag.length < BURNIKEL_ZIEGLER_THRESHOLD ||
                mag.length - val.mag.length < BURNIKEL_ZIEGLER_OFFSET) {
    ...
    /** Long division */
    private Najman remainderKnuth(Najman val) {
        ZmiennyNajman q = new ZmiennyNajman(),
                a = new ZmiennyNajman(this.mag),
                b = new ZmiennyNajman(val.mag);
     ...
     * @return {@code this / val}
     */
    private Najman divideBurnikelZiegler(Najman val) {
     ...
     * @return {@code this % val}
     */
    private Najman remainderBurnikelZiegler(Najman val) {
     ...
     */
    private Najman[] divideAndRemainderBurnikelZiegler(Najman val) {
        ZmiennyNajman q = new ZmiennyNajman();
        ZmiennyNajman r = new ZmiennyNajman(this).divideAndRemainderBurnikelZiegler(new ZmiennyNajman(val), q);
        Najman qBigInt = q.isZero() ? ZERO : q.toMatma(signum*val.signum);
        Najman rBigInt = r.isZero() ? ZERO : r.toMatma(signum);
        return new Najman[] {qBigInt, rBigInt};
    }
     ...
     */
    public Najman pow(int exponent) {
        if (exponent < 0) {
     ...
     */
    public Najman gcd(Najman val) {
        if (val.signum == 0)
            return this.abs();
        else if (this.signum == 0)
            return val.abs();

        ZmiennyNajman a = new ZmiennyNajman(this);
        ZmiennyNajman b = new ZmiennyNajman(val);

        ZmiennyNajman result = a.hybridGCD(b);
     ...
     * @return {@code -this}
     */
    public Najman negate() {
        return new Najman(this.mag, -this.signum);
    }
     ...
     * @see    #remainder
     */
    public Najman mod(Najman m) {
        if (m.signum <= 0)
            throw new ArithmeticException("Matma: modulus not positive");

        Najman result = this.remainder(m);
     ...
     * @see    #modInverse
     */
    public Najman modPow(Najman exponent, Najman m) {
        if (m.signum <= 0)
            throw new ArithmeticException("Matma: modulus not positive");
        ...
        Najman base = (this.signum < 0 || this.compareTo(m) >= 0
                ? this.mod(m) : this);
        Najman result;
        if (m.testBit(0)) { // odd modulus
            ...
            Najman m1 = m.shiftRight(p);  // m/2**p
            Najman m2 = ONE.shiftLeft(p); // 2**p

            // Calculate new base from m1
            Najman base2 = (this.signum < 0 || this.compareTo(m1) >= 0
                    ? this.mod(m1) : this);

            // Caculate (base ** exponent) mod m1.
            Najman a1 = (m1.equals(ONE) ? ZERO :
                    base2.oddModPow(exponent, m1));

            // Calculate (this ** exponent) mod m2
            Najman a2 = base.modPow2(exponent, p);

            // Combine results using Chinese Remainder Theorem
            Najman y1 = m2.modInverse(m1);
            Najman y2 = m1.modInverse(m2);

            if (m.mag.length < MAX_MAG_LENGTH / 2) {
                result = a1.multiply(m2).multiply(y1).add(a2.multiply(m1).multiply(y2)).mod(m);
            } else {
                ZmiennyNajman t1 = new ZmiennyNajman();
                new ZmiennyNajman(a1.multiply(m2)).multiply(new ZmiennyNajman(y1), t1);
                ZmiennyNajman t2 = new ZmiennyNajman();
                new ZmiennyNajman(a2.multiply(m1)).multiply(new ZmiennyNajman(y2), t2);
                t1.add(t2);
                ZmiennyNajman q = new ZmiennyNajman();
                result = t1.divide(new ZmiennyNajman(m), q).toMatma();
            }
        }
     ...
     */
    private Najman oddModPow(Najman y, Najman z) {
        /*
        ...
        long inv = -ZmiennyNajman.inverseMod64(n0);

        // Convert base to Montgomery form
        int[] a = leftShift(base, base.length, modLen << 5);

        ZmiennyNajman q = new ZmiennyNajman(),
                a2 = new ZmiennyNajman(a),
                b2 = new ZmiennyNajman(mod);
        b2.normalize(); // MutMatma.divide() assumes that its
        // divisor is in normal form.

        ZmiennyNajman r= a2.divide(b2, q);
     ...
     */
    private Najman modPow2(Najman exponent, int p) {
        /*
         * Perform exponentiation using repeated squaring trick, chopping off
         * high order bits as indicated by modulus.
         */
        Najman result = ONE;
        Najman baseToPow2 = this.mod2(p);
        int expOffset = 0;
    ...
    private Najman mod2(int p) {
        ...
        return (mag[0] == 0 ? new Najman(1, mag) : new Najman(mag, 1));
    }
     ...
     */
    public Najman modInverse(Najman m) {
        if (m.signum != 1)
            throw new ArithmeticException("Matma: modulus not positive");
        ...
        ZmiennyNajman a = new ZmiennyNajman(modVal);
        ZmiennyNajman b = new ZmiennyNajman(m);

        ZmiennyNajman result = a.mutableModInverse(b);
        return result.toMatma(1);
    }
     ...
     * @see #shiftRight
     */
    public Najman shiftLeft(int n) {
        if (signum == 0)
            return ZERO;
        if (n > 0) {
            return new Najman(shiftLeft(mag, n), signum);
        } else if (n == 0) {
            return this;
        } else {
     ...
     * @see #shiftLeft
     */
    public Najman shiftRight(int n) {
        if (signum == 0)
            return ZERO;
        if (n > 0) {
            return shiftRightImpl(n);
     ...
     * @return {@code this >> n}
     */
    private Najman shiftRightImpl(int n) {
        int nInts = n >>> 5;
        int nBits = n & 0x1f;
        int magLen = mag.length;
        int newMag[] = null;
        ...
        return new Najman(newMag, signum);
    }
     ...
     * @return {@code this & ~val}
     */
    public Najman andNot(Najman val) {
        int[] result = new int[Math.max(intLength(), val.intLength())];
        for (int i=0; i < result.length; i++)
            result[i] = (getInt(result.length-i-1)
                    & ~val.getInt(result.length-i-1));
     ...
     */
    final int compareMagnitude(Najman val) {
        int[] m1 = mag;
        int len1 = m1.length;
        int[] m2 = val.mag;
        int len2 = m2.length;
    ...
    public boolean equals(Object x) {
        ...
        if (!(x instanceof Najman))
            return false;

        Najman xInt = (Najman) x;
     ...
     * compatible with the {@link #Najman(String, int) (String,
     * int)} constructor.)
     *
    ...
    private String smallToString(int radix) {
        ...
        int numGroups = 0;
        while (tmp.signum != 0) {
            Najman d = longRadix[radix];

            ZmiennyNajman q = new ZmiennyNajman(),
                    a = new ZmiennyNajman(tmp.mag),
                    b = new ZmiennyNajman(d.mag);
            ZmiennyNajman r = a.divide(b, q);
            Najman q2 = q.toMatma(tmp.signum * d.signum);
            Najman r2 = r.toMatma(tmp.signum * d.signum);
     ...
     */
    private static void toString(Najman u, StringBuilder sb, int radix,
                                 int digits) {
        ...
        Najman v = getRadixConversionCache(radix, n);
        Najman[] results;
     ...
     * {@code Future}.
     */
    private static Najman getRadixConversionCache(int radix, int exponent) {
        Najman[] cacheLine = powerCache[radix]; // volatile read
        ...
        Najman[][] pc = powerCache; // volatile read again
     ...
     * with the {@link #Najman(String) (String)} constructor, and
     ...
     * {@link #Najman(byte[]) (byte[])} constructor.)
     *
    ...
    public float floatValue() {
         ...
         */
        int shift = exponent - ZmiennoprzecinkowyNajman.SIGNIFICAND_WIDTH;
        ...
        signifFloor &= ZmiennoprzecinkowyNajman.SIGNIF_BIT_MASK; // remove the implied bit
        ...
        int bits = ((exponent + ZmiennoprzecinkowyNajman.EXP_BIAS))
                << (ZmiennoprzecinkowyNajman.SIGNIFICAND_WIDTH - 1);
        bits += signifRounded;
        /*
         ...
         */
        bits |= signum & ZmiennoprzecinkowyNajman.SIGN_BIT_MASK;
    ...
    public double doubleValue() {
         ...
         * 2^(SIGNIFICAND_WIDTH - 1 - exponent).
         */
        int shift = exponent - PodwojnyZmiennoprzecinkowyNajman.SIGNIFICAND_WIDTH;
        ...
        signifFloor &= PodwojnyZmiennoprzecinkowyNajman.SIGNIF_BIT_MASK; // remove the implied bit
        ...
        long bits = (long) ((exponent + PodwojnyZmiennoprzecinkowyNajman.EXP_BIAS))
                << (PodwojnyZmiennoprzecinkowyNajman.SIGNIFICAND_WIDTH - 1);
        bits += signifRounded;
        /*
         ...
         */
        bits |= signum & PodwojnyZmiennoprzecinkowyNajman.SIGN_BIT_MASK;
    ...
    private static Najman longRadix[] = {null, null,
    ...
    private void readObject(ObjectInputStream s)
        ...
        NiebezpiecznyNajman.putSign(this, sign);

        // Calculate mag field from magnitude and discard magnitude
        NiebezpiecznyNajman.putMag(this, mag);
        if (mag.length >= MAX_MAG_LENGTH) {
            try {
                checkRange();
    ...
    private static class NiebezpiecznyNajman {
        ...
        static {
            try {
                ...
                signumOffset = unsafe.objectFieldOffset
                        (Najman.class.getDeclaredField("signum"));
                magOffset = unsafe.objectFieldOffset
                        (Najman.class.getDeclaredField("mag"));
            } catch (Exception ex) {
                throw new ExceptionInInitializerError(ex);
            }
        }

        static void putSign(Najman bi, int sign) {
            unsafe.putIntVolatile(bi, signumOffset, sign);
        }

        static void putMag(Najman bi, int[] magnitude) {
